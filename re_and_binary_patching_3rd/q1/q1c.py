def fix_message_data(data: bytes) -> bytes:
    ...
    last_value_xored = values_to_xor[-1]
    accumulator_xors ^= last_value_xored
    accumulator_xors &= 0xFF
    
    patch_last_value = (accumulator_xors ^ verification_value) & 0xFF

    lst_data[length + metadata_length - 1] = patch_last_value 
